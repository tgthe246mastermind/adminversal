...
function SocialDashboard() {
  const API_BASE = import.meta.env.VITE_API_URL; // must point to your worker base
  const [currentPlatform, setCurrentPlatform] = useState("Instagram");

  // Live Facebook stats (loaded from worker)
  ...
  useEffect(() => {
    const loadFacebook = async () => {
      ...
      try {
        ...
        if (!json?.connected) {
          setFbStats(null);
          setFbError("No Facebook Page connected yet.");
        } else {
          setFbStats(json);
        }
      } catch (e) {
  ...
  // Your dummy values remain for non-Facebook platforms (until you wire them too)
  const platformData = useMemo(() => {
    ...
    // If Facebook live data exists, map it into your existing 3 fields:
    if (fbStats?.success && fbStats?.connected) {
      ...
      base.Facebook = {
        ...
        posts: postsThisWeek,
        changes: {
          followers: fbLoading ? "Same" : "Same",
          engagement: fbLoading ? "Same" : "Same",
          posts: fbLoading ? "Same" : "Same",
        },
      };
    }
  ...
  return (
    <div className="tab-content active" id="social-dashboard">
      ...
      {/* Facebook status line (only shows on FB tab) */}
      {currentPlatform === "Facebook" && (
        <div style={{ textAlign: "center", marginBottom: 10 }}>
          {fbLoading && <span>Loading Facebook metrics…</span>}
          {!fbLoading && fbError && <span style={{ color: "red" }}>{fbError}</span>}
          {!fbLoading && !fbError && fbStats?.page?.name && (
            <span>Connected: {fbStats.page.name}</span>
          )}
        </div>
      )}
